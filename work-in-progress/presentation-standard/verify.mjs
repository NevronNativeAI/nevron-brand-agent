...
const numbers=[...new Set([...Object.values(layouts),...Array.from({length:10},(_,i)=>i+5),54,55])].sort((a,b)=>a-b);
const results=[];
for(const number of numbers){
  const name='house-'+String(number).padStart(2,'0'),json=path.join(out,'capture.json');
  fs.writeFileSync(json,JSON.stringify({title:`House slide ${number}`,mediaRoot:path.resolve(mediaRoot),fontDir:process.env.NEVRON_FONT_DIR,slides:[{houseSlide:number}]}));
  build(json,out,{reference:true,fontMode:process.env.NEVRON_FONT_MODE||"open-sans"});
  ...
  results.push({houseSlide:number,capture:name+'.png',reference:`reference-${number}.png`,diagnostics});console.log(`${name}: captured; ${diagnostics.length} diagnostics`);
}
fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(results,null,2));
fs.writeFileSync(path.join(out,'index.html'),`<!doctype html><html lang="en"><meta charset="utf-8"><title>House-slide comparison</title><style>body{margin:24px;background:#ddd;font:16px sans-serif}article{margin-bottom:32px}figure{margin:0}main{display:grid;grid-template-columns:1fr 1fr;gap:8px}img{display:block;width:100%}summary{cursor:pointer}pre{white-space:pre-wrap}</style><h1>House-slide comparison</h1><p>Left: exported house PNG (1280×720). Right: extracted HTML rendered at 1600×900. Open Sans is the requested substitution. No visual-match pass is inferred from these captures.</p>${results.map(r=>`<article><h2>House ${r.houseSlide}</h2><main><figure><figcaption>PowerPoint export</figcaption><img src="${r.reference}"></figure><figure><figcaption>Extracted HTML · Open Sans</figcaption><a href="${r.capture}"><img src="${r.capture}"></a></figure></main><details><summary>${r.diagnostics.length} text/image diagnostics</summary><pre>${JSON.stringify(r.diagnostics,null,2)}</pre></details></article>`).join('')}</html>`);
