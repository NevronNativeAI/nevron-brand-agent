...
import { pluginRoot, libraryRoot } from '../../lib/brand-paths.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PLUGIN  = pluginRoot(__dirname);   // bundled: logos, icons, tokens
const LIBRARY = libraryRoot(PLUGIN);     // repo clone: illustrations, screenshots (may be null)
...
function logoSVG(name, colour) {
  const svg = readFirst([path.join(PLUGIN, 'assets', 'logos', name)]);
  if (!svg) {
...
function iconHTML(spec) {
  ...
  const tries = [
    ...
    path.join(PLUGIN, 'assets', 'icons', spec),
    path.join(PLUGIN, 'assets', 'icons', 'contentware', spec),
    path.join(PLUGIN, 'assets', 'icons', 'contentware', spec + '.svg'),
    // illustrations are not bundled - only reachable when the repo clone is present
    LIBRARY && path.join(LIBRARY, 'assets', 'illustrations', spec),
  ];
...
function renderBlocks(blocks, opts) {
  ...
  blocks.forEach((b, i) => {
    ...
    // A row of captioned photos, two up. A service report is mostly evidence,
    // and one photo per page turns a four-photo report into four pages.
    if (b.figures !== undefined) {
      const figs = b.figures || [];
      // How many sit on a row is a judgement about the photographs, not a
      // fixed rule: one wide rack shot needs the full width, four detail
      // shots read fine two or three up. Default to what fits, let the
      // author override with `per`, and cap at four before it stops being
      // legible on paper.
      const per = Math.min(4, Math.max(1, Number(b.per) || (figs.length <= 3 ? figs.length : 2)));
      const fit = b.fit === 'contain' ? ' figgrid--contain' : '';
      const cells = figs.map((f) => {
        const src = localFile(typeof f === 'string' ? f : f.img);
        const cap = typeof f === 'string' ? '' : (f.caption || '');
        const fig = `<figure class="figure"><img src="${src}" alt=""></figure>`;
        const sub = cap ? `<p class="Imagesubtext">${inline(cap)}</p>` : '';
        return `<div class="figgrid__cell">${fig}${sub}</div>`;
      }).join('');
      out.push(`<div class="figgrid figgrid--${per}${fit}">${cells}</div>`);
      return;
    }
